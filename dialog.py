...
class LoginDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.nameEdit.textEdited.connect(lambda: checkNameEdit(self))
    ...
    def disableEdit(self):
        ...
        self.nameEdit.setText('anonymous')
...
class BaseProgressWidget(QWidget):
    ...
    def __init__(self, text='', parent=None):
        ...
        self.bottomBorder = QWidget(self)
    ...
    def set_value(self, value):
        ...
        if self.totalValue >= self.progressbar.maximum():
            self.destroy()
...
class ProgressDialog(QMainWindow):
    ...
    def addProgress(self, type, title, size):
        if type not in ['download', 'upload']:
            raise Exception("type must 'download' or 'upload'")
...
def checkNameEdit(login_dialog: LoginDialog):
    if not login_dialog.nameEdit.text() and not login_dialog.visitorRadio.isChecked():
        login_dialog.buttonBox.button(QDialogButtonBox.Ok).setEnabled(False)
    elif login_dialog.nameEdit.text() and login_dialog.registerRadio.isChecked():
        login_dialog.buttonBox.button(QDialogButtonBox.Ok).setEnabled(True)
